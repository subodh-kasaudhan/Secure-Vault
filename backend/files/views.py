        ...
        try:
            ...
            with transaction.atomic():
                ...
                if created:
                    ...
                    self._move_temp_to_blob_storage(temp_path, blob.path)
                else:
    ...
    def _move_temp_to_blob_storage(self, temp_path, blob_relative_path):
        ...
        if 'CLOUDINARY_URL' in os.environ or getattr(settings, 'DEFAULT_FILE_STORAGE', '').endswith('MediaCloudinaryStorage'):
            ...
            try:
                ...
                # Upload file to Cloudinary with the blob path as the public_id
                # Cloudinary will handle the storage
                result = cloudinary.uploader.upload(
                    temp_path,
                    folder='secure-vault/blobs',
                    public_id=blob_relative_path.replace('blobs/', '').replace('/', '_'),
                    resource_type='auto',
                ...
                # Note: With Cloudinary, the path stored in database is the Cloudinary public_id
                # The actual URL will be generated by Cloudinary storage backend
                return
            except Exception as e:
